...
class TenantRegistry:
    ...
    def get(self, tenant_id: str, use_cache: bool = True) -> TenantConfig:
        ...
        tenant = TenantConfig(
            ...
            whatsapp_provider=data.get("whatsapp_provider"),
            whatsapp_instance_url=data.get("whatsapp_instance_url"),
            whatsapp_api_key=data.get("whatsapp_api_key"),
            whatsapp_instance_name=data.get("whatsapp_instance_name"),
        )
    ...
    async def get_async(self, tenant_id: str, use_cache: bool = True) -> TenantConfig:
        """
        Busca configuração de um tenant do Supabase de forma assíncrona.
        Wraps the synchronous get() method via asyncio.to_thread.
        ...
        # Delegate to synchronous get() in a thread to avoid blocking the event loop
        import asyncio
        return await asyncio.to_thread(self.get, tenant_id, use_cache)
